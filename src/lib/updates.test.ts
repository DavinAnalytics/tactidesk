...
import { isMissingReleaseError, previewStatusFromQuery, updateBannerText } from "./updates";
...
describe("update banner copy", () => {
  ...
  it("treats a missing GitHub Release as a quiet miss", () => {
    expect(isMissingReleaseError({ statusCode: 404, message: "Not Found" })).toBe(true);
    expect(isMissingReleaseError(new Error("Cannot find channel latest.yml"))).toBe(true);
    expect(isMissingReleaseError(new Error("ENOTFOUND"))).toBe(false);
  });
});
